...
class TemplateService {
  ...
  async getTemplate(id: string): Promise<MessageTemplate> {
    const response = await apiClient.get<MessageTemplate>(ENDPOINTS.TEMPLATES.BY_ID(id));
    return response.data;
  }

  async createTemplate(data: CreateTemplateInput): Promise<MessageTemplate> {
    const response = await apiClient.post<MessageTemplate>(ENDPOINTS.TEMPLATES.BASE, data);
    return response.data;
  }

  async updateTemplate(id: string, data: UpdateTemplateInput): Promise<MessageTemplate> {
    const response = await apiClient.put<MessageTemplate>(ENDPOINTS.TEMPLATES.BY_ID(id), data);
    return response.data;
  }
